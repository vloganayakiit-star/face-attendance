...
def init_db():
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS students (
        ...
        name TEXT, department TEXT, year TEXT,
        roll_no TEXT, email TEXT, phone TEXT,
        enrolled_at TEXT)""")
...
def add_student(sid,name,dept,year,roll,email,phone):
    ...
    try:
        conn.execute(
            ...
            (sid,name,dept,year,roll,email,phone,
        ...
        with st.form("add_form", clear_on_submit=True):
            ...
            if submitted:
                if not f_id.strip() or not f_name.strip():
                    ...
                    if ok:
                        st.success("✅ "+f_name+" ("+f_id+") added successfully!")
                        add_log("New student added: "+f_name+" ("+f_id+")","SUCCESS")
                    else:
                        st.error(msg)
            ...
            if bulk_input.strip():
                ...
                for line in lines:
                    ...
                    if len(parts) >= 4:
                        ...
                        ok,_  = add_student(sid,sname,sdept,syear,sroll,semail,sphone)
                        if ok:
                            success += 1
                        else:
                            failed += 1
                        ...
                        failed += 1
                st.success("✅ "+str(success)+" students added successfully!")
                if failed > 0:
                    st.warning(str(failed)+" rows skipped (duplicate ID or wrong format)")
